...
path_app_stats = './data/path_app_user.json'
path_user_inventory = './data/user_inventory_sample.txt'

engine = create_engine('mysql+pymysql://root:<password>@localhost:3306/game_recommendation?charset=utf8mb4&local_infile=1') # local mysql server
...
df_steam_app.to_sql('tbl_app_info',engine,if_exists='replace',index=False, chunksize = 1000)
...
# change owners to be average_2week
df_popularity_based_results = pd.Series(list(dic_steamspy.get('average_2weeks').values()),list(dic_steamspy.get('average_2weeks').keys())).sort_values(ascending=False).to_frame()
